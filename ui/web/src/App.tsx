...
import { useMarketStoreConnected, useThrottledMarketRows } from "./store/useMarketStore";
...
export default function App() {
  const markets = useThrottledMarketRows(GROUP_RECOMPUTE_MS);
  ...
  const structureMarkets = useMemo(
    () => applyMarketFilters(markets, filters, groupNowMs, { keepNoLiquidity: true }),
    [markets, filters, groupNowMs],
  );
  ...
  const liveCount = useMemo(() => countLive(markets, nowMs), [markets, nowMs]);
  ...
  useEffect(() => {
    const id = window.setInterval(() => setGroupNowMs(Date.now()), GROUP_RECOMPUTE_MS);
      ...
      <div className="sticky top-0 z-50 -mx-4 mb-4 border-b border-ink-800/80 bg-[#0b1220]/95 px-4 py-2 backdrop-blur-sm sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-3 text-sm text-ink-400">
            <span className="inline-flex items-center gap-2 rounded-full border border-ink-700 bg-ink-900/80 px-3 py-1.5">
              <span className={`live-dot h-2 w-2 rounded-full ${connected ? "bg-accent" : "bg-amber-500"}`} />
              {state === "loading"
                ? "Connecting"
                : `${liveCount} live / ${markets.length} total`}
            </span>
            {updatedAt ? (
